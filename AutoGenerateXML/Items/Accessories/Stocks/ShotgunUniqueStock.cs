...
{
    public class ShotgunUniqueStock : StockXMLGenerator
    {
        public ShotgunUniqueStock() : base()
        {
            Name = nameof(ShotgunUniqueStock);
            Identifier = Identifiers.VGM_ShotgunUniqueStock;
...
$@"<Item name=""{Name.FollowedByModPrefix()}"" identifier=""{Identifier}"" category=""{Category}"" subcategory=""{SubCategory}""
    ...
    {GenerateItemSpriteXMLString(
        ...
        sourceRect: [0, 64, 49, 39],
        depth: 0.554f,
        origin: [0.5f, 0.5f]
    )}
    {GenerateRectangleBodyXMLString(width: 45, height: 35, density: 15)}
