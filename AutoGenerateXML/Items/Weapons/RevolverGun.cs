...
{
    public class RevolverGun : GunXMLGenerator
    {
        public RevolverGun() : base()
        {
            Name = "Revolver";
            OutputPath = Path.Combine("Guns", $@"{Name}.xml");
            Identifier = Identifiers.VGM_Revolver;

            SelfTags.AddRange(["smallitem", "pistolitem", "gunsmith", Name]);

            LowerAccessorySlotIndex = 1;
            UpperAccessorySlotIndex = 2;
        }
        ...
        public override List<ContainableAimingDevice> ContainableAimingDevices => [
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_RedDotSight], ItemPos: [2,12]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_HolographicSight], ItemPos: [0,12]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_ACOGScope], ItemPos: [-2,15]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_RifleScope], ItemPos: [0,12]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_SniperScope], ItemPos: [1,15]),
        ];
