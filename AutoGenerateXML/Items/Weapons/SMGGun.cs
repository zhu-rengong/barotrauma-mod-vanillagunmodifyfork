...
{
    public class SMGGun : GunXMLGenerator
    {
        public SMGGun() : base()
        {
            Name = "SMG";
            OutputPath = Path.Combine("Guns", $@"{Name}.xml");
            Identifier = Identifiers.VGM_SMG;

            SelfTags.AddRange(["smallautoweapon", "smallitem", "gunsmith", Name]);

            LowerAccessorySlotIndex = 1;
            MuzzleSlotIndex = 2;
            UpperAccessorySlotIndex = 3;
            ScannerSlotIndex = 4;
        }
        ...
        public override List<ContainableGrip> ContainableGrips => [
            new (GripXMLGenerator.All[Identifiers.VGM_AngledForeGrip], ItemPos: [20, -5]),
            new (GripXMLGenerator.All[Identifiers.VGM_VerticalGrip], ItemPos: [20, -7]),
        ];
        ...
        public override List<ContainableAimingDevice> ContainableAimingDevices => [
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_RedDotSight], ItemPos: [10,14]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_HolographicSight], ItemPos: [11,16]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_ACOGScope], ItemPos: [8,18]),
        ];

        public override List<ContainableScanner> ContainableScanners => [
            new(ScannerXMLGenerator.All[Identifiers.VGM_HealthScanner], ItemPos: [19,10]),
            new(ScannerXMLGenerator.All[Identifiers.VGM_ThermalScanner], ItemPos: [19,10]),
        ];
