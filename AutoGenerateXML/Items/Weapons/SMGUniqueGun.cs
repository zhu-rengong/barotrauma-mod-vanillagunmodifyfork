...
{
    public class SMGUniqueGun : GunXMLGenerator
    {
        public SMGUniqueGun() : base()
        {
            Name = "SMGUnique";
            OutputPath = Path.Combine("Guns", $@"{Name}.xml");
            Identifier = Identifiers.VGM_SMGUnique;

            SelfTags.AddRange(["smallautoweapon", "smallitem", Name]);
        ...
        public override List<ContainableGrip> ContainableGrips => [
            new(GripXMLGenerator.All[Identifiers.VGM_AngledForeGrip], ItemPos: [10,-2]),
            new(GripXMLGenerator.All[Identifiers.VGM_VerticalGrip], ItemPos: [11,-4]),
            new(GripXMLGenerator.All[Identifiers.VGM_BipodGrip], ItemPos: [13,-12]),
        ];

        public override List<ContainableStock> ContainableStocks => [
            new(StockXMLGenerator.All[Identifiers.VGM_SMGUniqueStock], ItemPos: [-34,4]),
            new(StockXMLGenerator.All[Identifiers.VGM_LightStock], ItemPos: [-41,4]),
            new(StockXMLGenerator.All[Identifiers.VGM_LightSniperStock], ItemPos: [-39,5]),
            new(StockXMLGenerator.All[Identifiers.VGM_LightWrenchStock], ItemPos: [-39,4]),
        ];
        ...
        public override List<ContainableAimingDevice> ContainableAimingDevices => [
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_RedDotSight], ItemPos: [-13,16]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_HolographicSight], ItemPos: [-9,17]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_ACOGScope], ItemPos: [-13,19]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_RifleScope], ItemPos: [-10,16]),
            new(AimingDeviceXMLGenerator.All[Identifiers.VGM_SniperScope], ItemPos: [-9,18]),
        ];

        public override List<ContainableScanner> ContainableScanners => [
            new(ScannerXMLGenerator.All[Identifiers.VGM_HealthScanner], ItemPos: [3,11]),
            new(ScannerXMLGenerator.All[Identifiers.VGM_ThermalScanner], ItemPos: [3,11]),
        ];
